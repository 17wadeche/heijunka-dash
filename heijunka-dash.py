...
st.markdown("---")
st.markdown("---")
st.subheader("Available vs Completed Hours — by Output and by UPLH")
...
hours_bars = (
    ...
    .encode(
        ...
        color=alt.Color("Hours Type:N", title=""),
        tooltip=[
            "team:N",
            "period_date:T",
            "Hours Type:N",
...
# Output line (Actual Output)
output_line = (
    alt.Chart(f.dropna(subset=["Actual Output"]))
    .mark_line(point=True)
    .encode(
        x=alt.X("period_date:T", title="Week"),
        y=alt.Y("Actual Output:Q", title="Output"),
...
# UPLH line (Actual UPLH)
uplh_line = (
    alt.Chart(f.dropna(subset=["Actual UPLH"]))
    .mark_line(point=True)
    .encode(
        x=alt.X("period_date:T", title="Week"),
        y=alt.Y("Actual UPLH:Q", title="UPLH"),
        color=alt.value("#9467bd"),
        tooltip=[
            "team:N",
            "period_date:T",
            alt.Tooltip("Actual UPLH:Q", format=",.2f")
        ],
        detail="team:N"
    )
    .properties(height=120)
...
# Build two composites: bars (hours) over the same x, with a small chart under for Output / UPLH
left_combo = alt.vconcat(
    hours_bars.properties(title="Available vs Completed Hours (context: Actual Output)"),
    output_line,
).resolve_scale(x="shared")

right_combo = alt.vconcat(
    hours_bars.properties(title="Available vs Completed Hours (context: Actual UPLH)"),
    uplh_line,
).resolve_scale(x="shared")

c1, c2 = st.columns(2)
with c1:
    st.altair_chart(left_combo, use_container_width=True)
with c2:
    st.altair_chart(right_combo, use_container_width=True)
